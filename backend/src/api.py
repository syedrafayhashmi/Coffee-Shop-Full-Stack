...
def create_drink(payload):
    ...
    new_drink = Drink(
        title=body['title'], recipe="""{}""".format(body['recipe']))
    new_drink.insert()
...
def update_drink(payload, drink_id):
    ...
    return jsonify({'success': True, 'drinks': [Drink.long(drink_to_update)]})
...
def delete_drinks(payload, drink_id):
    ...
    if not drink_to_delete:
        abort(404, {'message': """Drink with
        id {} not found in database.""".format(drink_id)})
...
def unprocessable(error):
    return jsonify({
        "success": False,
        "error": 422,
        "message": get_error_message(error, "unprocessable")}), 422
